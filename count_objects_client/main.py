...
while True:
    ...
    frame_kanal = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV) [:, :, :3]

    frame_gray = cv2.cvtColor(frame_kanal, cv2.COLOR_BGR2GRAY)
    frame_inv = cv2.threshold(frame_gray, 100, 255, cv2.THRESH_BINARY)[1]

    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (10, 10))
    # thresh = cv2.dilate(frame_inv, kernel, iterations=2)
    thresh = cv2.erode(frame_inv, kernel, iterations=3)
